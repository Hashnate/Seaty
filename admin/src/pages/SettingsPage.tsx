  ...
  return (
    <div>
      ...
      <div className="table-card" style={{ maxWidth: '700px', marginTop: '20px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {[
            { name: 'FastAPI Backend', url: 'http://localhost:8000/api/v1' },
            { name: 'WebSockets GPS Hub', url: 'ws://localhost:8000/api/v1/ws' },
            { name: 'PostgreSQL Database', url: 'Localhost Postgres Pool' },
          ].map((srv, idx) => (
            <div key={idx} style={{
              display: 'flex', justifyContent: 'space-between', alignItems: 'center',
